...
| 复制 API Key        | 已复制 API Key            | API Key 复制失败，请手动复制  |
| 复制 OpenAI 示例    | 已复制 OpenAI 示例代码    | 示例代码复制失败，请手动复制  |
| 复制 Anthropic 示例 | 已复制 Anthropic 示例代码 | 示例代码复制失败，请手动复制  |
| 复制 Codex 示例     | 已复制 Codex 示例代码     | 示例代码复制失败，请手动复制  |
...
| ------------------------ | -------------------------------------------------------- |
| 验证开始                 | 正在验证当前接入配置，请稍候...                          |
| 验证成功                 | 接入验证成功，当前 Key 可正常请求共享容量服务。          |
| 验证失败（鉴权）         | 接入验证失败：请检查 API Key 是否正确或是否已过期。      |
...
- 若复制后仍无法请求，请优先检查 Base URL、API Key 和模型名称是否与页面示例一致。
- 当产品池处于降级状态时，验证失败不一定意味着当前 Key 本身不可用。
...
- 无可用 Key：当前项目暂无可用于接入的 Key，请联系运营分配。
